...
Eleven slides, approximately 11–16 minutes. The 20 September version is reserved for Iteration 3 testing.
...
Official main at commit 2f0d932 was checked on 25 September 2026. The Diagrams and Artifacts folder contains only a readme. The midterm documents include templates but no completed use case diagram. The root README is a heading. This limits traceability: we can test observed behavior and written cases, but actors, boundaries and alternate flows need agreement with the development group. We should request the approved use case diagram and functional requirements, then update the RTM. This is a documentation gap, not one of the seven confirmed application defects. Source: https://github.com/APC-SoCIT/APC_2026_2027_T1_SS231_G02-Jazgottours/tree/main
...
We recommend the booking data defects, UC-003-BUG-001 and UC-003-BUG-002, as the first priority. They change what the customer appears to purchase and what the dashboard confirms. They were proposed as high severity in the report. The PDF defect, UC-002-BUG-001, is next because it prevents delivery of the invoice and blocks the content test. The old checkout form accepted malformed card values under UC-002-BUG-002, a high severity finding for that tested version. However, the newer version replaces that form with a PayMongo redirect. We will retire the obsolete card field assertion and test the new gateway flow in a controlled sandbox in Iteration 3. Priority describes urgency for the next fixes and tests; the proposed severities and owners still need development team triage.

## 9. Remaining issues

Other findings also need a decision. UC-001-BUG-001 records that the booking form accepted a past tour date. SETUP-BUG-001 records a clean Windows install failing on a Linux-only Tailwind dependency; a forced install let us continue QA but did not change that case result. UC-004-BUG-001 records a React hydration error on the quotation page. All five admin routes returned page content, so this was not a blank page. UC-004-SCOPE-001 asks whether quotation submission and record creation belong to the agreed requirement. The page had a preview without a save or submit action. Unused Supabase integration was only OBS-001 in the tested version, not a confirmed customer defect. The new checkout API changes backend use, so that observation also needs fresh assessment in Iteration 3.
